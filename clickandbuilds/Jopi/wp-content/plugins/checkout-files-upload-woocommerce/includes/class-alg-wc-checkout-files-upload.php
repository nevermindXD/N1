...
class Alg_WC_Checkout_Files_Upload_Main {
		...
		if ( 'yes' === get_option( 'alg_wc_checkout_files_upload_enabled' ) ) {
			...
			for ( $i = 1; $i <= $total_number; $i++ ) {
				if ( 'disable' != ( $the_hook = get_option( 'alg_checkout_files_upload_hook_' . $i, 'woocommerce_before_checkout_form' ) ) ) {
					add_action( $the_hook, array( $this, 'add_files_upload_form_to_checkout_frontend' ), get_option( 'alg_checkout_files_upload_hook_priority_' . $i, 10 ) );
				}
	...
	function validate_on_checkout( $posted ) {
		...
		for ( $i = 1; $i <= $total_number; $i++ ) {
			if (
				...
				if ( '' != ( $file_accept = get_option( 'alg_checkout_files_upload_file_accept_' . $i, '' ) ) && isset( $_SESSION[ 'alg_checkout_files_upload_' . $i ] ) ) {
					// Validate file type
		...
		for ( $i = 1; $i <= $total_number; $i++ ) {
			if ( isset( $_POST[ 'alg_upload_checkout_file_' . $i ] ) ) {
				...
				if ( isset( $_FILES[ $file_name ] ) && '' != $_FILES[ $file_name ]['tmp_name'] ) {
					// Validate
					$is_valid = true;
					if ( '' != ( $file_accept = get_option( 'alg_checkout_files_upload_file_accept_' . $i, '' ) ) && isset( $_FILES[ $file_name ] ) ) {
						// Validate file type
						$file_accept = explode( ',', $file_accept );
						if ( is_array( $file_accept ) && ! empty( $file_accept ) ) {
	...
	function get_the_form( $i, $file_name, $order_id = 0 ) {
		...
		if ( '' != ( $the_label = get_option( 'alg_checkout_files_upload_label_' . $i, '' ) ) ) {
		...
		if ( '' == $file_name ) {
			$field_html = '<input type="file" name="alg_checkout_files_upload_' . $i . '" id="alg_checkout_files_upload_' . $i .
				'" accept="' . get_option( 'alg_checkout_files_upload_file_accept_' . $i, '' ) . '">';
			...
			$field_html = '<a href="' . add_query_arg( array( 'alg_download_checkout_file' => $i, '_wpnonce' => wp_create_nonce( 'alg_download_checkout_file' ) ) ) . '">' . $file_name . '</a>';
			$button_html = '<input type="submit"' .
				' class="button"' .
				' style="width:100%;"' .
	...
	function add_files_upload_form_to_checkout_frontend_all( $is_direct_call = false ) {
		...
		for ( $i = 1; $i <= $total_number; $i++ ) {
			$is_filter_ok = ( $is_direct_call ) ? true : (
				...
				$current_filter_priority == get_option( 'alg_checkout_files_upload_hook_priority_' . $i, 10 )
			);
